...
@section('content')
                    ...
                    @else
                        @foreach($steps as $step)
                            <div class="content-item d-flex align-items-center border rounded p-2 mb-2" data-index="{{ $loop->index }}" style="transition: all 0.3s ease;">
                                ...
                                <button type="button" class="btn btn-sm px-3" onclick="removeContent(this)" title="Excluir conteúdo" style="background-color: {{ old('plan_color', $track->plan_color ?? '#06D6A0') }}; color: white;">
                                    Excluir
                                </button>
                            </div>
                        @endforeach
    ...
    <script>
        ...
        function addUrlContent() {
            ...
            let planColor = '{{ old('plan_color', $track->plan_color ?? '#06D6A0') }}';
